...
df <- ultimate_data %>%
  select(c('turns_per_game', 'ds_per_game', 'ast_per_game', 'pts_per_game', 'pls_mns_per_game'))
corrplot::corrplot(cor(df), method = 'circle')
lm_model <- lm(pls_mns_per_game ~ turns_per_game + ds_per_game + ast_per_game + pts_per_game, data = df)
summary(lm_model)


vif_results <- car::vif(lm_model)
vif_df <- data.frame(Variable = names(vif_results),
                     VIF = vif_results)

vif_df$Variable <- factor(vif_df$Variable,levels = c('turns_per_game', 'ds_per_game', 'ast_per_game', 'pts_per_game'),labels = c("Turns per game", "Ds per game", "Assists per game", "Points per game"))
