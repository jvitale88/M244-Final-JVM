...
def ridge_by_div_gender(ultimate_data, alpha=1.0, test_size=0.2, random_state=42):
    ...
    for (division, gender), group in ultimate_data.groupby(['division', 'gender']):
...
for (division, gender), res in resultsdg.items():
    print(f"{division} {gender} — R²: {res['score (R^2)']:.3f}")
...
def plot_feature_contributions(results):
    ...
    # For each group (division, gender), plot the feature coefficients
    for (division, gender), res in results.items():
        ...
        plt.title(f"Feature Contributions: {division} {gender}")
        plt.tight_layout()
        plt.show()
```
