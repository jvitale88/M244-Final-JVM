...
```{python fit-lm-complete-dataset}
...
def linear_by_div_gender_cv(ultimate_data, cv=5):
    ...
    for (division, gender), group in ultimate_data.groupby(['level', 'gender']):
        ...
        if len(group_clean) < cv:
            continue  # Not enough samples for cross-validation
        ...
        # Cross-validated R²
        r2_scores = cross_val_score(model_pipeline, X, y, cv=cv, scoring='r2')
        mean_r2 = np.mean(r2_scores)

        # Cross-validated predictions for MAE and RMSE
        ...
        lin_model = model_pipeline.named_steps['linearregression']
        ...
        results_cv[(division, gender)] = {
            ...
            'mae': mae,
            'rmse': rmse,
            'coefficients': dict(zip(feature_cols, lin_model.coef_)),
...
ultimate_data[['turns_per_game', 'ds_per_game', 'ast_per_game', 'pts_per_game', 'pls_mns_per_game']].corr()
...
```{python fit-ridge-complete-dataset}
def ridge_reg(ultimate_data, alpha=1.0, test_size=0.2, random_state=42):
    ...
    # Number of obs (n) and predictors (p)
    n = len(y_test)  
    p = X_test.shape[1]
...
print("Adjusted R²:", results['adjusted (R^2)'])
```

```{python try-dropping-pts_per_game}
def ridge_reg2(ultimate_data, alpha=1.0, test_size=0.2, random_state=42):
    # Define features and target
...
results = ridge_reg2(ultimate_data)
...
def ridge_by_div_gender_cv(ultimate_data, alpha=1.0, cv=5):
    ...
    for (division, gender), group in ultimate_data.groupby(['level', 'gender']):
        ...
        # Create pipeline: scale, then fit Ridge regression
        ...
        y_pred_cv = cross_val_predict(model_pipeline, X, y, cv=cv)
        mae = mean_absolute_error(y, y_pred_cv)
        rmse = np.sqrt(mean_squared_error(y, y_pred_cv))  # RMSE calculation
...
```{python}
def lasso_by_div_gender_cv(ultimate_data, alphas=[0.0001, 0.001, 0.01, 0.1, 1, 10], cv=5, test_size=0.2, random_state=42):
    feature_cols = ['turns_per_game', 'ds_per_game', 'ast_per_game', 'pts_per_game']
    target_col = 'pls_mns_per_game'

    results = {}

    # Group by actual column values (level, gender)
    for (division, gender), group in ultimate_data.groupby(['level', 'gender']):
        group_clean = group[feature_cols + [target_col]].dropna()
        ...
        # Create pipeline: scale data then apply LassoCV
        model_pipeline = make_pipeline(StandardScaler(), LassoCV(alphas=alphas, cv=cv))
        ...
        # Fit on full data to get coefficients for interpretation
        model_pipeline.fit(X, y)
        lasso_model = model_pipeline.named_steps['lassocv']
        scaler = model_pipeline.named_steps['standardscaler']
...
def predict_plus_minus_for_player(ultimate_data, level, gender, player_stats, results_ridge_cv, resultsdg_cv, results_lasso_cv):
    ...
    if (level, gender) not in results_lasso_cv:
      ...
      # Print debug information to verify keys
    print(f"Available keys in Ridge Results: {list(results_ridge_cv.keys())}")
    print(f"Available keys in Linear Results: {list(resultsdg_cv.keys())}")
    print(f"Available keys in Lasso Results: {list(results_lasso_cv.keys())}")
    
     # Check if 'model' and 'scaler' exist for the given level and gender in the results
    ...
    ridge_model = results_ridge_cv[(level, gender)]['model']
    ridge_scaler = results_ridge_cv[(level, gender)]['scaler']
    
    linear_model = resultsdg_cv[(level, gender)]['model']
    linear_scaler = resultsdg_cv[(level, gender)]['scaler']
    
    lasso_model = results_lasso_cv[(level, gender)]['model']
    lasso_scaler = results_lasso_cv[(level, gender)]['scaler']
